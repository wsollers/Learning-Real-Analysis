...
f : A \to B,
\]
where $A$ is called the \emph{domain} of $f$ and $B$ the \emph{codomain}.
The set
\[
\{\, b \in B \mid \exists a \in A,\ f(a)=b \,\}
\]
is called the \emph{image} of $f$ and may be a proper subset of $B$.
\end{remark}

\begin{definition}[Preimage]
Let $f : A \to B$ be a function, and let $S \subseteq B$.
The \emph{preimage} (or \emph{inverse image}) of $S$ under $f$ is the set
\[
...
\begin{remark}
The notation $f^{-1}(S)$ does \emph{not} denote an inverse function.
It is defined for every function $f$, regardless of whether $f$ is injective or
bijective.

Preimages are fundamental because they are always well-defined and interact
cleanly with set operations.
\end{remark}

\begin{remark}
Preimages preserve set-theoretic structure:
...
\begin{remark}
Viewing functions via their graphs allows many properties of functions to be
proved using set-theoretic and relational arguments.
This perspective will be used implicitly in later sections.
\end{remark}
...
\begin{remark}
Injectivity concerns uniqueness of preimages, surjectivity concerns completeness
of images, and bijectivity combines both.
These properties govern whether functions admit inverses and how they behave
under composition and quotient constructions.
\end{remark}
...
Let $f : A \to B$ be a function.
A function $h : B \to A$ is called a \emph{right inverse} of $f$ if
\[
f \circ h = \operatorname{id}_B,
\]
that is,
\[
\forall b \in B,\quad f(h(b)) = b.
\]
\end{definition}
\begin{remark}[Inverses and Function Properties]
Let $f : A \to B$ be a function.
\begin{enumerate}[label=(\roman*)]
\item If $f$ has a left inverse, then $f$ is injective.
\item If $f$ has a right inverse, then $f$ is surjective.
\item If $f$ has both a left inverse and a right inverse, then they are equal,
and $f$ is bijective.
\end{enumerate}
\end{remark}
...
if $f : A \to B$, $g : B \to C$, and $h : C \to D$, then
\[
...
\begin{remark}[Interaction with Identity]
...
\begin{remark}[Injectivity and Surjectivity Under Composition]
Let
\[
f : A \to B
\quad\text{and}\quad
...
(The function $g$ need not be injective.)

\item If $g \circ f$ is surjective, then $g$ must be surjective.
(The function $f$ need not be surjective.)
\end{enumerate}
\end{remark}
...
\mathrm{id}_A : A \to A,
\qquad
\mathrm{id}_A(a)=a \text{ for all } a \in A.
\]
\end{definition}
...
Let $A$ and $B$ be sets with $A \subseteq A'$, and let
\[
f : A \to C
\quad\text{and}\quad
g : A' \to C
...
\begin{remark}
Functions interact naturally with equivalence relations.
A function $f : A \to B$ is said to be \emph{constant on equivalence classes}
of a relation $R$ on $A$ if
\[
(a,b)\in R \;\Longrightarrow\; f(a)=f(b).
\]
This condition is exactly what is required for $f$ to factor uniquely through
the quotient map $\pi : A \to A/R$.
\end{remark}

\begin{definition}[Image of a Set]
Let $f : A \to B$ be a function and let $S \subseteq A$.
The \emph{image} of $S$ under $f$ is
\[
f(S) := \{\, f(a) \mid a \in S \,\}.
\]
\end{definition}
