...
\subsection{Interval Arithmetic in $\mathbb{R}^n$}
...
\midrule
Box (product interval) & $[a,b] := \{x\in\mathbb{R}^n : a\le x \le b\}$ & \hyperref[def:box]{↓ Def} \\
Containment & $[a,b]\subseteq[c,d] \iff c\le a \ \land\ b\le d$ & \hyperref[def:box-contain]{↓ Def} \\
Minkowski sum & $[a,b]\oplus[c,d]=[a+c,\; b+d]$ & \hyperref[def:box-add]{↓ Def} \\
Difference & $[a,b]\ominus[c,d]=[a-d,\; b-c]$ & \hyperref[def:box-sub]{↓ Def} \\
Scalar mult. $(\lambda\ge 0)$ & $\lambda[a,b]=[\lambda a,\; \lambda b]$ & \hyperref[def:box-smul]{↓ Def} \\
Scalar mult. $(\lambda<0)$ & $\lambda[a,b]=[\lambda b,\; \lambda a]$ & \hyperref[def:box-smul]{↓ Def} \\
Coordinatewise product & $[a,b]\odot[c,d]=\prod_i [\min S_i,\max S_i]$ & \hyperref[def:box-hadamard]{↓ Def} \\
Norm bound (Euclidean) & $\|x\|_2 \le \max\{\|a\|_2,\|b\|_2\}$ for $x\in[a,b]$ if $0\le a\le b$ & \hyperref[rem:norm-bounds]{↓ Rem} \\
\bottomrule
\end{tabular}
\end{tcolorbox}
