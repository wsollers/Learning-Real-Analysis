...
\begin{remark*}[Associated inductive subset]
Given a predicate $\varphi$ on $P$, define
\[
A=\{n \in P : \varphi(n)\}.
\]
Then the base case $\varphi(1)$ is exactly the statement $1 \in A$, and the
successor step
\[
\forall n \in P\;(\varphi(n) \Longrightarrow \varphi(S(n)))
\]
is exactly the statement that $A$ is closed under successor. The theorem is
obtained by applying the induction axiom to this subset.
\end{remark*}
\end{topicbox}
...
\begin{tcolorbox}[colback=thmbox, colframe=thmborder, arc=2pt,
  left=6pt, right=6pt, top=4pt, bottom=4pt,
  title={\small\textbf{Theorem (Predecessor Existence and Uniqueness Away from \texorpdfstring{$1$}{1})}} ,
  fonttitle=\small\bfseries]
