...
\[
\forall L, \exists n \in \mathbb{N} \cup \{0\} : \bigl( \operatorname{Length}(L) = n \land L = (x_1, x_2, \dots, x_n) \bigr)
\]
\end{remark*}
...
\begin{itemize}
    \hfill \item \textbf{Order Sensitivity}: The lists $(a, b)$ and $(b, a)$ are distinct unless $a=b$. \hfill
    \hfill \item \textbf{Finiteness}: By Axler's convention, a list must have a finite number of elements. Infinite ordered collections are categorized as sequences. \hfill
    \hfill \item \textbf{Empty List}: A list of length $n=0$ is called the empty list, denoted by $()$. \hfill
\end{itemize}
\end{remark*}
