...
\[
  ...
  \exists! r \in \mathbb{Z},\; \exists q \in \mathbb{Z},\;
  (a = qn + r \;\wedge\; 0 \le r < n).
\]
\end{remark*}

\begin{remark*}[Flash]
\FlashQ{Define $a \bmod n$.}
\FlashA{The unique $r\in[0,n)$ with $a=qn+r$ for some $q\in\mathbb{Z}$. Equivalently $r = a - n\lfloor a/n\rfloor$.}
\FlashHint{Remainder is always in $[0,n)$ even for negative $a$.}
\end{remark*}
...
\begin{remark*}[Flash]
\FlashQ{State idempotence of $\bmod$.}
\FlashA{$(a\bmod n)\bmod n = a\bmod n$: once reduced, reducing again does nothing.}
\FlashHint{$a\bmod n\in[0,n)$ already, so a second reduction is the identity.}
\end{remark*}
...
\begin{remark*}[Flash]
\FlashQ{Define congruence modulo $n$.}
\FlashA{$a\equiv b\pmod{n}$ iff $n\mid(a-b)$, equivalently $a$ and $b$ have the same remainder mod $n$.}
\FlashHint{$a\equiv b\pmod{n}\iff\exists k\in\mathbb{Z}, a=b+kn$.}
\end{remark*}
...
\begin{remark*}[Flash]
\FlashQ{Define the congruence class $[a]_n$.}
\FlashA{$[a]_n = \{a+kn : k\in\mathbb{Z}\}$ --- all integers congruent to $a$ modulo $n$.}
\FlashHint{$\mathbb{Z}/n\mathbb{Z}$ has exactly $n$ congruence classes: $[0]_n,\ldots,[n-1]_n$.}
\end{remark*}
...
\label{thm:congruence-equivalence}
...
\[
  ...
  \begin{cases}
    a \equiv a \pmod{n} \\
    a \equiv b \Rightarrow b \equiv a \pmod{n} \\
    a \equiv b \wedge b \equiv c \Rightarrow a \equiv c \pmod{n}
  \end{cases}
\]
\end{remark*}

\begin{remark*}[Flash]
\FlashQ{Is congruence mod $n$ an equivalence relation?}
\FlashA{Yes: reflexive ($a\equiv a$), symmetric ($a\equiv b\Rightarrow b\equiv a$), transitive ($a\equiv b, b\equiv c\Rightarrow a\equiv c$).}
\FlashImplies{def:congruence-class}
\end{remark*}
...
\label{thm:congruence-compatibility}
...
\begin{remark*}[Flash]
\FlashQ{State compatibility of congruence with arithmetic.}
\FlashA{If $a\equiv b$ and $c\equiv d\pmod{n}$, then $a\pm c\equiv b\pm d$ and $ac\equiv bd\pmod{n}$.}
\FlashHint{Representatives may be swapped before arithmetic: the class of the result is unchanged.}
\FlashImplies{thm:congruence-equivalence}
\end{remark*}
