...
\label{def:mathematical-induction}
...
\[
  ...
  \forall S \subseteq \mathbb{Z},\;
  \Bigl[
    a \in S
    \;\wedge\;
    (\forall n \ge a,\; n \in S \Rightarrow n+1 \in S)
    \;\Rightarrow\;
    \forall n \ge a,\; n \in S
  \Bigr].
\]
\end{remark*}

\begin{remark*}[Flash]
\FlashQ{State the Principle of Mathematical Induction.}
\FlashA{If $a\in S\subseteq\mathbb{Z}$ and $n\in S\Rightarrow n+1\in S$ for all $n\ge a$, then $\{n\in\mathbb{Z}:n\ge a\}\subseteq S$.}
\FlashHint{Base case + inductive step $\Rightarrow$ universal conclusion.}
\end{remark*}

\begin{definition}[Mathematical Induction --- Logical Form]
\label{def:induction-logical-form}
...
\begin{remark*}[Flash]
\FlashQ{State the logical form of mathematical induction.}
\FlashA{$P(a) \wedge (\forall n\ge a, P(n)\Rightarrow P(n+1)) \Rightarrow \forall n\ge a, P(n)$.}
\FlashHint{Predicate version: the set $S$ is replaced by the extension of $P$.}
\end{remark*}
...
\[
  ...
  \Bigl[
    \forall n \ge a,\;
    (\forall k,\; a \le k < n \Rightarrow k \in S)
    \Rightarrow n \in S
  \Bigr]
  \;\Rightarrow\;
  \forall n \ge a,\; n \in S.
...
\begin{remark*}[Flash]
\FlashQ{State strong induction.}
\FlashA{If assuming all cases $a\le k<n$ are in $S$ implies $n\in S$, then $\{n\ge a\}\subseteq S$.}
\FlashHint{Strong: assume ALL previous cases. Weak: assume only the immediately previous case.}
\FlashImplies{def:mathematical-induction}
\end{remark*}
