...
\begin{proof}
If $\mathcal{A}$ is an algebra, then $X \in \mathcal{A}$ by (i), and
$A \setminus B = A \cap B^c \in \mathcal{A}$ since $B^c \in \mathcal{A}$
by (ii) and $A \cap B^c = (A^c \cup B)^c \in \mathcal{A}$ by closure under
complements and union. So $\mathcal{A}$ is a ring containing $X$.

Conversely, if $\mathcal{R}$ is a ring with $X \in \mathcal{R}$, then for
any $A \in \mathcal{R}$ we have $A^c = X \setminus A \in \mathcal{R}$ by
the ring's closure under $\setminus$. So $\mathcal{R}$ is an algebra.
\end{proof}
...
\begin{proof}
Countable intersections: $\bigcap_{n=1}^\infty A_n
= \left(\bigcup_{n=1}^\infty A_n^c\right)^c \in \mathcal{F}$
by closure under complements and countable unions.
Set difference: $A \setminus B = A \cap B^c \in \mathcal{F}$
by closure under complements and (finite) intersection.
Finite Boolean combinations follow from the algebra property, which is
implied by the $\sigma$-algebra axioms (as $\sigma$-algebra implies algebra).
\end{proof}
