...
% ---------------------------------------------------------
\subsubsection{Working with the Definition: A First Example}

Before proving any theorem, it is worth unpacking the definition completely
on a concrete case.

\begin{tcolorbox}[colback=gray!6, colframe=gray!40, arc=2pt,
  left=6pt, right=6pt, top=4pt, bottom=4pt,
  title={\small\textbf{Example: Verifying $x_n = 1/n \to 0$ from the $\varepsilon$-definition}},
  fonttitle=\small\bfseries]
\textbf{Claim.} $\displaystyle\lim_{n\to\infty}\frac{1}{n} = 0$.

\medskip
\textbf{What we must show.}
$\forall \varepsilon > 0,\; \exists N \in \mathbb{N},\; \forall n \ge N:\;
\left|\frac{1}{n} - 0\right| < \varepsilon$.

\medskip
\textbf{Finding $N$.}
We need $\tfrac{1}{n} < \varepsilon$, i.e.\ $n > \tfrac{1}{\varepsilon}$.
By the Archimedean property, there exists $N \in \mathbb{N}$ with
$N > \tfrac{1}{\varepsilon}$.

\medskip
\textbf{Verification.}
For any $n \ge N$: $n \ge N > \tfrac{1}{\varepsilon}$, so
$\tfrac{1}{n} \le \tfrac{1}{N} < \varepsilon$.
Therefore $\left|\tfrac{1}{n} - 0\right| = \tfrac{1}{n} < \varepsilon$. \checkmark

\medskip
\textbf{Quantifier pattern (explicit).}
\[
\underbrace{\forall \varepsilon > 0}_{\text{adversary chooses}}\;
\underbrace{\exists N := \lceil 1/\varepsilon \rceil}_{\text{we respond}}\;
\underbrace{\forall n \ge N}_{\text{then for all}}\;:
\quad \left|\tfrac{1}{n}\right| < \varepsilon.
\]
Notice that $N$ depends on $\varepsilon$ --- this is expected and correct.
The game is: \emph{whatever} $\varepsilon$ is chosen, we can always find a
suitable $N$.
\end{tcolorbox}
...
\begin{proposition}[Bounded Does Not Imply Convergent]
