...
\begin{tcolorbox}[colback=gray!6, colframe=gray!40, arc=2pt,
  ...
  title={\small\textbf{Algebra Of Sequences — Quick Reference}},
...
\begin{tcolorbox}[colback=propbox, colframe=propborder, arc=2pt,
  left=6pt, right=6pt, top=4pt, bottom=4pt,
  title={\small\textbf{Definition (Algebraic operations)}},
  fonttitle=\small\bfseries]
...
\begin{remark*}[The add-and-subtract technique]
The product rule requires bounding $|x_n y_n - xy|$.
There is no obvious way to split this into pieces involving $|x_n - x|$
and $|y_n - y|$ separately --- until we add and subtract $x_n y$:
\[
x_n y_n - xy
= x_n y_n - x_n y + x_n y - xy
= x_n(y_n - y) + y(x_n - x).
\]
Now each factor involves only one of the errors $|y_n - y|$ or $|x_n - x|$,
which we know how to make small.

\medskip
This \textbf{add-and-subtract} maneuver --- introduce a carefully chosen
zero ($\pm x_n y$ above) to split a single error into two manageable pieces
--- is a recurring technique throughout analysis.
You will see it again in proofs involving continuity, uniform convergence,
and integration.
When stuck on a product error, try inserting $\pm (\text{one factor fixed,
one varying})$.
\end{remark*}
...
\begin{corollary}[Limit Respects Upper Bounds]
If $x_n \le b$ for all $n$ and $x_n \to x$, then $x \le b$.
\end{corollary}

\begin{corollary}[Convergent Times Bounded Is Bounded]
If $(x_n)$ converges and $(y_n)$ is bounded, then the product sequence
$(x_n y_n)$ is bounded.
\end{corollary}

\begin{corollary}[Convergence of Absolute Values]
