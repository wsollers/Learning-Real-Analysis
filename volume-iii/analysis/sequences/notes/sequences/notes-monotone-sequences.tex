...
\begin{remark*}[This theorem depends on completeness]
The Monotone Convergence Theorem asserts that a bounded increasing sequence
has a limit equal to $\sup\{x_n\}$.  That supremum exists as a real number
only because of the \textbf{Least Upper Bound Property}
(Completeness Axiom, Bounding chapter).

Over $\mathbb{Q}$, this theorem fails: the sequence of rational approximations
to $\sqrt{2}$ is increasing and bounded above in $\mathbb{Q}$, yet has no
rational limit.  Completeness is not optional here.

\medskip
\textbf{Cross-reference:} the Completeness Axiom and the
$\varepsilon$-characterization of the supremum are stated and proved in
the Bounding chapter (\texttt{notes-completeness.tex} and
