...
% ---------------------------------------------------------
% ---------------------------------------------------------
\subsubsection{Why Do We Need Limsup and Liminf?}
% ---------------------------------------------------------

\begin{remark*}[Motivation: what limsup and liminf are for]
The ordinary limit $\lim_{n \to \infty} x_n$ is powerful when it exists, but many
bounded sequences do not converge.  For example, the sequence
\[
x_n = (-1)^n\bigl(1 + 1/n\bigr)
\]
oscillates and has no limit.  Yet there is clearly some structure to its
long-term behavior: the terms approach $+1$ from above on even indices and
$-1$ from below on odd indices.

\medskip

\textbf{The gap:} $\lim$ either exists as a single number or fails entirely.
We need weaker quantities that \emph{always} exist for bounded sequences and
capture the ``eventual ceiling'' and ``eventual floor'' of the sequence.

\medskip

\textbf{The answer:} $\limsup$ and $\liminf$.
\begin{itemize}
  \item $\limsup_{n \to \infty} x_n$ is the \emph{eventual upper bound}: the
        smallest number $L$ such that $x_n > L$ only finitely many times.
        Equivalently, it is the limit of the shrinking tail suprema
        $s_n = \sup\{x_k : k \ge n\}$.
  \item $\liminf_{n \to \infty} x_n$ is the \emph{eventual lower bound}: the
        largest number $l$ such that $x_n < l$ only finitely many times.
\end{itemize}

For a convergent sequence, $\limsup = \liminf = \lim$.
For a divergent bounded sequence, $\limsup > \liminf$, and the gap between
them measures how much the sequence oscillates.

\medskip

\textbf{Why this matters:} The Ratio Test and Root Test use $\limsup$
because the ratio/root may not converge, but $\limsup$ always exists.
The Bolzano--Weierstrass theorem can be rephrased: $\limsup$ and $\liminf$
are themselves subsequential limits (the largest and smallest ones).
\end{remark*}
