...
Let $f:[a,b]\to\mathbb{R}$ be continuous and let $\varepsilon>0$. There exists a step function $s_\varepsilon:[a,b]\to\mathbb{R}$ such that $\sup_{x\in[a,b]} |f(x)-s_\varepsilon(x)|<\varepsilon$.
...
\coloneqq
\Bigl(s_\varepsilon \text{ is a step function on } [a,b]\Bigr)
\land
...
\Bigl(s \text{ step function } \Rightarrow
...
\begin{tcolorbox}[colback=defbox, colframe=defborder, arc=2pt,
  left=6pt, right=6pt, top=4pt, bottom=4pt,
  title={\small\textbf{Definition (Bernstein Polynomial)}},
  fonttitle=\small\bfseries]
\begin{definition}[Bernstein Polynomial]
\label{def:bernstein-polynomial}
Let n\in\mathbb{N} and f:[0,1]\to\mathbb{R}. A function B_n:[0,1]\to\mathbb{R} is the nth Bernstein polynomial of f if and only if for every x\in[0,1],
\[
...
\begin{aligned}
&\text{Let }n\in\mathbb{N},\; f:[0,1]\to\mathbb{R},\; B_n:[0,1]\to\mathbb{R}.\\
&\neg\Bigl(B_n \text{ is the } n\text{th Bernstein polynomial of } f\Bigr)\\
&\Longleftrightarrow
\exists x\in[0,1]\;
\Bigl(
B_n(x)\neq\sum_{k=0}^n f(k/n)\binom{n}{k}x^k(1-x)^{n-k}
\Bigr).
\end{aligned}
\]
\end{remark*}
...
\begin{remark*}[Negation predicate reading]
\[
\neg\operatorname{BernsteinPolynomial}(B_n,f,n)
\Longleftrightarrow
\neg\bigl(B_n:[0,1]\to\mathbb{R}\bigr)
\lor
\exists x\in[0,1]\;
\Bigl(
B_n(x)\neq\sum_{k=0}^n f(k/n)\binom{n}{k}x^k(1-x)^{n-k}
...
\begin{remark*}[Failure modes]
Failure occurs either because $B_n$ is not a function from $[0,1]$ to
$\mathbb{R}$, or because at some point $x\in[0,1]$ its value does not
match the prescribed binomial average of the sample values $f(k/n)$.
\end{remark*}

\begin{remark*}[Failure mode decomposition]
\[
\neg\operatorname{BernsteinPolynomial}(B_n,f,n)=
\underbrace{\neg\bigl(B_n:[0,1]\to\mathbb{R}\bigr)}_{\text{wrong function type}}
\;\lor\;
\underbrace{\exists x\in[0,1]\;
\Bigl(
B_n(x)\neq\sum_{k=0}^n f(k/n)\binom{n}{k}x^k(1-x)^{n-k}
\Bigr)}_{\text{Mismatch with binomial averaging at some }x}.
\]
\end{remark*}
