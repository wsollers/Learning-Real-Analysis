...
\[
\begin{aligned}
&\text{Let } I \subseteq \mathbb{R} \text{ be an interval, } f:I\to\mathbb{R} \text{ increasing, and } c\in I \text{ with } \exists a,b\in I \text{ such that } a<c<b.\\
&\text{Let } L_- = \sup\{f(x): x\in I,\; x<c\} \text{ and } L_+ = \inf\{f(x): x\in I,\; c<x\}.\\
...
&\Bigl[\operatorname{Interval}(I)\land \operatorname{InteriorPoint}(c,I)
\land \operatorname{Increasing}(f,I)
\land f(c^-)\text{ exists}\land f(c^+)\text{ exists}\Bigr]\\
&\qquad\Rightarrow
...
\[
\operatorname{JumpOfFunction}(f,c,j)
\coloneqq
j=f(c^+)-f(c^-).
\]
\end{remark*}
...
\begin{remark*}[Negation predicate reading]
\[
\neg\operatorname{JumpOfFunction}(f,c,j)
\Longleftrightarrow
\neg\bigl(f(c^-)\text{ exists}\bigr)
\lor
\neg\bigl(f(c^+)\text{ exists}\bigr)
...
\Bigl[
a\ne b\land
\neg\operatorname{ContinuousAt}(f,a)\land
\neg\operatorname{ContinuousAt}(f,b)
\Rightarrow
