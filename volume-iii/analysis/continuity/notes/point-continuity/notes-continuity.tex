...
\begin{remark*}[Failure modes]
Continuity at $c$ fails exactly when one can construct a sequence inside $A$ converging to $c$ whose image sequence fails to converge to $f(c)$, providing a witness to discontinuity.
\end{remark*}

\begin{remark*}[Failure mode decomposition]
\[
\underbrace{\lim_{n\to\infty} x_n = c}_{\text{approach } c \text{ inside } A}
\quad\text{together with}\quad
\underbrace{\lim_{n\to\infty} f(x_n) \ne f(c)}_{\text{output deviates from } f(c)}
\]
\end{remark*}
