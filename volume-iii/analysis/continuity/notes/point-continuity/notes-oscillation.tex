...
The only failure mode for a proposed oscillation value is that it does
not equal the supremum of all pairwise differences of function values on
$E$.
\end{remark*}

\begin{remark*}[Failure mode decomposition]
\[
\underbrace{\omega\ne\sup\{\,|f(x_1)-f(x_2)|:x_1,x_2\in E\,\}}_{\text{incorrect oscillation magnitude}}.
\]
\end{remark*}
...
\land \Bigl(\inf_{\delta>0}\sup\{\cdots\}>0\Bigr)\\
&\qquad \lor\\
&\Bigl(\exists \varepsilon_0>0 \; \forall \delta>0 \; \exists x \in A \; (|x-c|<\delta \land |f(x)-f(c)| \ge \varepsilon_0)\Bigr)
\land \Bigl(\inf_{\delta>0}\sup\{\cdots\}=0\Bigr),
\end{aligned}
\]
where $\sup\{\cdots\}$ abbreviates $\sup\{|f(x)-f(y)| : x,y \in A,\; |x-c|<\delta,\; |y-c|<\delta\}$.
\end{remark*}
