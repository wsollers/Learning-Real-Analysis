...
\bigl(\operatorname{Within}(x,a,\delta) \Rightarrow |f(x)-f(a)| < \varepsilon\bigr)\Bigr).
\end{aligned}
\]
\end{remark*}
...
\bigl(\operatorname{Within}(x,a,\delta) \land |f(x)-f(a)| \ge \varepsilon\bigr).
\end{aligned}
\]
\end{remark*}
...
\bigl(\operatorname{Within}(x,a,\delta) \Rightarrow |f(x)-f(a)| < \varepsilon\bigr).
\]
\end{remark*}
...
\begin{remark*}[Failure modes]
Discontinuity fails exactly when every neighborhood $V$ of $f(c)$ admits some neighborhood $U$ of $c$ whose intersection with $A$ is mapped entirely inside $V$; this is the continuity condition at $c$.
\end{remark*}
...
\neg \operatorname{DiscontinuousAt}(f,c)
=
\underbrace{\forall V \ni f(c)\; \exists U \ni c\; \forall x \in U \cap A\; (f(x) \in V)}_{\text{Continuity at } c}.
\]
\end{remark*}
...
\[
\begin{aligned}
&\text{Within the same context as above:}\\
...
\text{(Not essential)} &\Longleftrightarrow \operatorname{RemovableDiscontinuity}(f,x_0).
\end{aligned}
\]
\end{remark*}
...
\begin{remark*}[Dependencies]
\hyperref[def:point-of-discontinuity]{Definition~\ref*{def:point-of-discontinuity}}
% UNRESOLVED_DEPENDENCY: Bilateral function limit at a point | Reason: Label not available in index.
% UNRESOLVED_DEPENDENCY: Left and right limits at a point | Reason: Labels not available in index.
\end{remark*}
