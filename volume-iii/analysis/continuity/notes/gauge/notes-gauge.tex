...
\[
\operatorname{PartitionOfInterval}(P,I) \coloneqq \bigl(P=\{x_0,\ldots,x_n\}\subseteq I \land x_0=a \land x_n=b \land \forall i\in\{1,\ldots,n\}\;(x_{i-1}<x_i)\bigr).
\]
\end{remark*}
...
\[
\neg\operatorname{PartitionOfInterval}(P,I) \Longleftrightarrow (x_0\ne a) \lor (x_n\ne b) \lor \exists i\in\{1,\ldots,n\}\;(x_{i-1}\ge x_i).
\]
\end{remark*}
...
&\Longleftrightarrow
\Bigl(\exists j \in \{1,\ldots,n\}\;(x_{j-1} \ge x_j) \lor \exists i \in \{1,\ldots,n\}\;(t_i \notin [x_{i-1},x_i])\Bigr).
\end{aligned}
\]
\end{remark*}
...
\begin{remark*}[Failure mode decomposition]
\[
\underbrace{\exists j\;(x_{j-1} \ge x_j)}_{\text{partition defect}}
\;\lor\;
...
\begin{remark*}[Failure modes]
A purported gauge fails precisely when it takes on a nonpositive value somewhere in $I$.
\end{remark*}
