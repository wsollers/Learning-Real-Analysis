...
\bigl(
s=\sup A \rightarrow \forall x \in A \;(x \le s)
\bigr).
\]
\end{remark*}

\begin{remark*}[Proposition predicate reading]
\[
\forall A \subseteq \mathbb{R}\; \forall s \in \mathbb{R}\;
\bigl(
\operatorname{Supremum}(s,A) \rightarrow \operatorname{UpperBound}(s,A)
\bigr).
\]
\end{remark*}
...
\bigl(
s=\sup A \land \exists x \in A \;(s < x)
\bigr).
\]
\end{remark*}
...
s=\sup A
\land
\underbrace{\exists x \in A \;(s < x)}_{\text{$s$ fails to lie above every element of $A$}}
\Bigr).
\]
\end{remark*}
...
\begin{remark*}[Failure modes]\hfill
\begin{itemize}
  \item \textbf{Failure of upper-bound control.} There exists an element of $A$ strictly larger than $s$.
  \item \textbf{Failure of the supremum condition.} Any such example contradicts the fact that a supremum is an upper bound of the set.
\end{itemize}
\end{remark*}

\begin{remark*}[Interpretation]
This proposition isolates the first part of the meaning of supremum. A supremum is not only the least among upper bounds; it is itself an upper bound. Therefore every element of the set must lie below it.
\end{remark*}
