...
\begin{enumerate}[label=(\roman*)]
  ...
  \item Periodic: $\sin(x+2\pi) = \sin x$;\;
    $\cos(x+2\pi) = \cos x$.
\end{enumerate}
\end{remark*}
...
connecting trigonometric and exponential functions at the deepest
level. Euler's identity $e^{i\pi} + 1 = 0$ follows immediately.
\end{remark*}
...
For $0 < x < \pi/2$, the area comparison of two triangles and a
circular sector gives $\cos x \leq \sin x / x \leq 1$. Since $\cos
x \to 1$ as $x \to 0^+$, the Squeeze Theorem gives $\sin x / x \to
1$. The even symmetry of $\sin x / x$ extends this to $x \to 0^-$.
\end{remark*}

\begin{remark*}[Significance]
This is the statement $(\sin x)'\big|_{x=0} = 1$, which implies
$(\sin x)' = \cos x$ everywhere. It underlies all trigonometric
differentiation and is used in the proof of the addition formulas
via the limit approach.
\end{remark*}
