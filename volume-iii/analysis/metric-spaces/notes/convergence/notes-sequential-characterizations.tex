...
% ---------------------------------------------------------
% THE UNIFYING THEOREM — boxed centrepiece
% ---------------------------------------------------------
...
\begin{tcolorbox}[
  ...
  title={\small\textbf{The Unifying Theorem: Three Languages for Closure}},
  fonttitle=\small\bfseries
]
\label{thm:closure-three-way-unifying}
...
\begin{tabular}{c l l}
\textbf{(T)} & \textbf{Topological:} & $x \in \overline{E}$ \\
\textbf{(G)} & \textbf{Geometric:}   & $\forall r > 0,\; B_r(x) \cap E \neq \varnothing$ \\
\textbf{(S)} & \textbf{Sequential:}  & $\exists (a_n) \subseteq E$ with $a_n \to x$
\end{tabular}
\end{center}
...
This equivalence is the first deep theorem that ties the three fundamental
perspectives of analysis together:

\begin{itemize}
  \item The \emph{topological} language asks about membership in a constructed set ($\overline{E}$).
  \item The \emph{geometric} language asks about balls and intersections.
  \item The \emph{sequential} language asks about limits of sequences.
\end{itemize}
These three languages are freely interchangeable in metric spaces.
Proofs can switch between them at will, choosing whichever is most convenient.

In topology (the next chapter), only the geometric and topological descriptions
survive --- there may be no metric and hence no sequences.
Here, in metric spaces, all three are available and equivalent.

\medskip
\noindent\textit{Proof: Theorem~\ref{thm:closure-equivalences} and
Proposition~\ref{prf:sequential-characterization-closure}.}
\end{tcolorbox}

\input{volume-iii/analysis/metric-spaces/notes/convergence/figure-closure-three-way}
