...
In particular, closed and open sets admit useful sequential
characterizations that link the metric geometry of the space
...
The set $F$ is \emph{closed} if every convergent sequence of
points of $F$ converges to a point of $F$.
\end{definition}
...
\begin{definition}[Open Set — Sequential Characterization]
\label{def:open-sequential}
Let $(X,d)$ be a metric space and let $U \subseteq X$.

The set $U$ is \emph{open} if every sequence that converges
to a point of $U$ eventually lies entirely in $U$.
\end{definition}
...
\item $F$ is closed (i.e.\ $X \setminus F$ is open).
...
The three conditions highlight different perspectives.
\begin{itemize}
