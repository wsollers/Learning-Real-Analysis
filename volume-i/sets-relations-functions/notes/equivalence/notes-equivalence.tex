...
\begin{proof}
($\Rightarrow$) Assume $[a] = [b]$. Since $a \in [a]$, we have $a \in [b]$,
so $(b,a) \in R$. By symmetry, $(a,b) \in R$.

($\Leftarrow$) Assume $(a,b) \in R$.

\emph{$[a] \subseteq [b]$:} Let $x \in [a]$, so $(a,x) \in R$. Since
$(b,a) \in R$ (by symmetry) and $(a,x) \in R$, transitivity gives
$(b,x) \in R$, so $x \in [b]$.
...
\begin{proof}
\textbf{(i)} We verify the three partition conditions for $A/R$.

\emph{Nonemptiness.} For any $a \in A$, reflexivity gives $(a,a) \in R$, so
$a \in [a]_R \neq \varnothing$.

\emph{Covering.} For any $x \in A$, reflexivity gives $x \in [x]_R \subseteq
\bigcup A/R$.

\emph{Disjointness.} Suppose $[a]_R \cap [b]_R \neq \varnothing$; pick
$x \in [a]_R \cap [b]_R$. Then $(a,x) \in R$ and $(b,x) \in R$. By symmetry
$(x,b) \in R$, and transitivity gives $(a,b) \in R$. By the Representative
Independence Lemma, $[a]_R = [b]_R$.

\textbf{(ii)} We check reflexivity, symmetry, and transitivity for
$R_{\mathcal{P}}$.

\emph{Reflexive.} Any $a \in A$ lies in some block $P$ (covering), so
$(a,a) \in R_{\mathcal{P}}$.

\emph{Symmetric.} If $a,b \in P$ then $b,a \in P$, so symmetry is immediate.

\emph{Transitive.} If $a,b \in P$ and $b,c \in Q$, then $b \in P \cap Q \neq
\varnothing$, so $P = Q$ by disjointness. Hence $a,c \in P$ and
$(a,c) \in R_{\mathcal{P}}$.

\textbf{(iii)} For $R \mapsto A/R \mapsto R_{A/R}$: $(a,b) \in R_{A/R}$
iff $a,b$ lie in a common block $[x]_R$, which by the Representative
Independence Lemma is equivalent to $(a,b) \in R$.

For $\mathcal{P} \mapsto R_{\mathcal{P}} \mapsto A/R_{\mathcal{P}}$: for
each $a \in A$ let $P_a$ be the unique block containing $a$. One verifies
$[a]_{R_{\mathcal{P}}} = P_a$, so $A/R_{\mathcal{P}} = \mathcal{P}$.
\end{proof}
