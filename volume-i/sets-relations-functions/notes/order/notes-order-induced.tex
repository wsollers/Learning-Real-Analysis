...
\begin{proof}
\emph{Reflexive.} For any $x \in A$, $f(x) \leq' f(x)$ by reflexivity of
$\leq'$, so $x \leq_f x$.

\emph{Transitive.} Suppose $x \leq_f y$ and $y \leq_f z$. Then
$f(x) \leq' f(y)$ and $f(y) \leq' f(z)$, so $f(x) \leq' f(z)$ by
transitivity of $\leq'$. Hence $x \leq_f z$.
\end{proof}
...
\begin{proof}
We already know $\leq_f$ is a preorder (Proposition~\ref{prop:induced-preorder}).
It remains to check antisymmetry.

\medskip
$(\Rightarrow)$ Suppose $\leq_f$ is antisymmetric. Let $x, y \in A$ with
$f(x) = f(y)$. Then $f(x) \leq' f(y)$ and $f(y) \leq' f(x)$ (by
reflexivity of $\leq'$, since they are equal), so $x \leq_f y$ and
$y \leq_f x$. Antisymmetry of $\leq_f$ gives $x = y$. Hence $f$ is
injective.

\medskip
$(\Leftarrow)$ Suppose $f$ is injective. Assume $x \leq_f y$ and
$y \leq_f x$. Then $f(x) \leq' f(y)$ and $f(y) \leq' f(x)$. By
antisymmetry of $\leq'$, $f(x) = f(y)$. Since $f$ is injective, $x = y$.
Hence $\leq_f$ is antisymmetric.
\end{proof}
...
\begin{proof}
Suppose $f(x) = f(y)$. Then $f(x) \leq' f(y)$ and $f(y) \leq' f(x)$
(by reflexivity of $\leq'$). By order-reflection, $x \leq y$ and $y \leq x$.
By antisymmetry of $\leq$, $x = y$.
\end{proof}
...
\begin{proof}
$f$ is injective by Proposition~\ref{prop:embedding-injective}, and
it maps $A$ onto $f(A)$ by definition, so $f : A \to f(A)$ is a bijection.
For $x, y \in A$:
\[
x \leq y
\;\Longleftrightarrow\;
...
f(x) \leq'_{f(A)} f(y),
\]
where the first equivalence is the order embedding condition and the second
holds because $f(x), f(y) \in f(A)$ and the suborder agrees with $\leq'$
on elements of $f(A)$. Hence $f$ is an order isomorphism onto $f(A)$.
\end{proof}
