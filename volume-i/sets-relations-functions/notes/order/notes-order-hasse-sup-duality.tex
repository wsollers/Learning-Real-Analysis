...
\begin{proposition}[Duality Principle]\label{thm:order-duality}
...
$\forall$ posets $(A,\leq)$, $\forall$ first-order sentences $\Phi$ in the language of $\leq$: $(A,\leq)\models\Phi \;\leftrightarrow\; (A,\geq)\models\Phi^*$, where $\Phi^*$ substitutes $\geq$ for every $\leq$.
\end{remark*}
