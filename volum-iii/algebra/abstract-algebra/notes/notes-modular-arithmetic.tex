...
\begin{proof}
Let $r = a \bmod n$. By definition, $0 \le r < n$.
Applying the Division Algorithm to $r$ divided by $n$,
\[
r = 0 \cdot n + r,
\]
since $0 \le r < n$ already. The remainder is $r$ itself.
Hence $(a \bmod n) \bmod n = r = a \bmod n$.
\end{proof}
...
\begin{proof}
\textbf{Reflexivity.}
Since $n \mid 0 = a - a$, we have $a \equiv a \pmod{n}$.

\medskip
\textbf{Symmetry.}
If $a \equiv b \pmod{n}$, then $n \mid (a - b)$.
Since $b - a = -(a - b)$, we have $n \mid (b - a)$,
so $b \equiv a \pmod{n}$.
...
a - c = (a - b) + (b - c),
\]
we have $n \mid (a - c)$, so $a \equiv c \pmod{n}$.
\end{proof}
...
\begin{proof}
Write $a = b + sn$ and $c = d + tn$ for integers $s, t$.
