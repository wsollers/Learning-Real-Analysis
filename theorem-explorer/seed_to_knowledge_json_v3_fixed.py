...
def strip_leading_proof_heading(text: str) -> str:
    t = text.strip()
    t = re.sub(r'^\\textbf\{[^{}]*?Proof\.?\}\.?(?:\\\\)?\s*', '', t, count=1).strip()
...
def load_seed(chapter_root: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    explorer_dir = chapter_root / ".explorer"
    knowledge_seed = json.loads((explorer_dir / "knowledge-seed.json").read_text(encoding="utf-8"))
    edge_seed_path = explorer_dir / "graph-edges-seed.json"
    edge_seed = json.loads(edge_seed_path.read_text(encoding="utf-8")) if edge_seed_path.exists() else []
...
def convert_chapter(chapter_root: Path, output_dir: Path | None = None) -> tuple[Path, Path, Path]:
    knowledge_seed, edge_seed = load_seed(chapter_root)
