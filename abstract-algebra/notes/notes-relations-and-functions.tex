...
\medskip
\noindent
\textbf{(⇒) An equivalence relation determines a partition.}
...
\begin{enumerate}
\item \textbf{Nonempty.}  
Since $R$ is reflexive, $a R a$, so $a \in [a]$.

\item \textbf{Union equals $S$.}  
Every $x \in S$ lies in $[x]$, hence
\[
\bigcup_{a \in S} [a] = S.
\]

\item \textbf{Disjoint or equal.}  
If $[a] \cap [b] \neq \emptyset$, choose $x$ in the intersection.
Then $xRa$ and $xRb$.
By symmetry $aRx$, and by transitivity $aRb$.
This implies $[a] \subseteq [b]$.
A symmetric argument gives $[b] \subseteq [a]$,
so $[a]=[b]$.
\end{enumerate}

Thus the equivalence classes form a partition.

\medskip
\noindent
\textbf{(⇐) A partition determines an equivalence relation.}
...
\text{$a$ and $b$ lie in the same part of } \mathcal{P}.
\]

\begin{itemize}
\item Reflexive: each element lies in its own part.
\item Symmetric: membership in the same part is symmetric.
\item Transitive: if $a,b$ and $b,c$ share a part, then so do $a,c$.
\end{itemize}
