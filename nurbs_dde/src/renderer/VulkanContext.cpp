...
void VulkanContext::init(GLFWwindow* window) {
    ...
    if (!inst_ret) {
        throw std::runtime_error("vk-bootstrap instance error: " + inst_ret.error().message());
    }
    ...
    features13.dynamicRendering = VK_TRUE;   // no render-pass objects needed
    ...
    if (!phys_ret) {
        throw std::runtime_error("vk-bootstrap physical device: " + phys_ret.error().message());
    }

    m_physical_device = phys_ret.value().physical_device;
    std::cout << "[VK] Physical device: " << phys_ret.value().properties.deviceName << "\n";

    // Logical device ───────────────────────────────────────────────────────────
    vkb::DeviceBuilder dev_builder{phys_ret.value()};
    auto dev_ret = dev_builder.build();
    if (!dev_ret) {
        throw std::runtime_error("vk-bootstrap device: " + dev_ret.error().message());
    }
    ...
    if (!gq || !pq) throw std::runtime_error("Failed to get Vulkan queues");
