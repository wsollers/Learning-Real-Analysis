// engine/Engine.cpp
// VOLK_IMPLEMENTATION defined exactly once here — the canonical TU.
#define VOLK_IMPLEMENTATION
#include <volk.h>

#include "engine/Engine.hpp"
#include "app/Scene.hpp"          // complete type — required before ~Engine body
...
// Both are defined here, not in the header, because Engine holds
// unique_ptr<Scene> and Scene is only forward-declared in Engine.hpp.
// unique_ptr requires a complete type at the point of destruction.

Engine::Engine() = default;

Engine::~Engine() {
    // Reverse construction order.
    m_scene.reset();          // Scene may hold font pointers — must die before renderer
    m_renderer.destroy();
...
void Engine::start(const std::string& config_path) {
    ...
    m_renderer.init(m_vk,
                    m_swapchain,
                    SHADER_DIR,
                    m_config.assets_dir,
                    m_glfw.window());

    m_buffer_manager.init(m_vk.device(),
...
void Engine::run_frame() {
    ...
    if (!m_renderer.begin_frame(m_swapchain)) {
        handle_resize();
        return;
    }
    ...
    if (!m_renderer.end_frame(m_swapchain))
        handle_resize();
}
...
void Engine::handle_resize() {
    ...
    std::cout << std::format("[Engine] Swapchain recreated {}x{}\n", w, h);
}
